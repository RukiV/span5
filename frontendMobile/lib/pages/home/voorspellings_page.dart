          ...
          children: [
            ...
            ValueListenableBuilder<List<Asset>>(
              ...
              builder: (context, assets, _) {
                if (assets.isEmpty) {
                  return const Text("Geen data beskikbaar", style: TextStyle(fontSize: 12, color: Colors.grey));
                }
                ...
                return DistributionBar(
                  segments: [
                    DistributionSegment(label: 'Aktief', color: AppColors.successGreen, flex: active),
                    DistributionSegment(label: 'Onderhoud', color: AppColors.warningOrange, flex: maintenance),
                    DistributionSegment(label: 'Afgedank', color: AppColors.errorRed, flex: retired),
                  ],
                );
              },
            ),
            ...
            ValueListenableBuilder<List<Report>>(
              ...
              builder: (context, reports, _) {
                final received = reports.where((r) => r.phase == "Ontvang").toList();
                if (received.isEmpty) {
                  return const Text("Geen data beskikbaar", style: TextStyle(fontSize: 12, color: Colors.grey));
                }
                int high = received.where((r) => r.priority == 'Hoog').length;
                int medium = received.where((r) => r.priority == 'Medium').length;
                int low = received.where((r) => r.priority == 'Laag').length;

                return DistributionBar(
                  segments: [
                    DistributionSegment(label: 'Hoog', color: AppColors.errorRed, flex: high),
                    DistributionSegment(label: 'Medium', color: AppColors.warningOrange, flex: medium),
                    DistributionSegment(label: 'Laag', color: AppColors.successGreen, flex: low),
                  ],
                );
              },
            ),
