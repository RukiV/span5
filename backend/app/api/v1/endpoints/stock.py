...
def addStock(stockIn: StockCreate, session: Session = Depends(getSession)):
    #Create new stock
    return stock_service.create(session, stockIn)

@router.patch("/{stockID}", response_model=StockRead)
def patchStock(stockID: int, stockIn: StockUpdate, session: Session = Depends(getSession)):
    #Update existing stock
    stock = stock_service.update(session, stockID, stockIn)
    if not stock:
        raise HTTPException(status_code=404, detail="Stock not found")
...
def removeStock(stockID: int, session: Session = Depends(getSession)):
    #Delete stock
    if not stock_service.delete(session, stockID):
        raise HTTPException(status_code=404, detail="Stock not found")
