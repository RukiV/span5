...
def addQuote(QuoteIn: QuoteCreate, session: Session = Depends(getSession), user: User = Depends(require_right("quotes.manage"))):
    #Create new quote
    return quote_service.create(session, QuoteIn, user_id=user.user_id)

@router.patch("/{quoteID}", response_model=QuoteRead)
def patchQuote(quoteID: int, QuoteIn: QuoteUpdate, session: Session = Depends(getSession), user: User = Depends(require_right("quotes.manage"))):
    #Update existing quote
    quote = quote_service.update(session, quoteID, QuoteIn, user_id=user.user_id)
    if not quote:
        raise HTTPException(status_code=404, detail="Quote not found")
...
def removeQuote(quoteID: int, session: Session = Depends(getSession), user: User = Depends(require_right("quotes.manage"))):
    #Delete quote
    if not quote_service.delete(session, quoteID, user_id=user.user_id):
        raise HTTPException(status_code=404, detail="Quote not found")
