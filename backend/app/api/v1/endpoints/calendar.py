...
def create_calendar_event(
    ...
    session: Session = Depends(getSession),
    user: User = Depends(require_right("calendar.manage")),
):
    user_id = user.user_id
    event = calendar_service.create(session, event_in, user_id=user_id)
    if event.notify_email and user.user_email:
        send_event_created(
...
def update_calendar_event(
    ...
    event = calendar_service.update(session, event_id, event_in, user_id=user.user_id)
    if not event:
        raise HTTPException(status_code=404, detail="Event nie gevind nie")
    return event
...
def delete_calendar_event(
    ...
    if not calendar_service.delete(session, event_id, user_id=user.user_id):
        raise HTTPException(status_code=404, detail="Event nie gevind nie")
